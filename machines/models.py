...
class License(models.Model):
    ...
    them: generate_license_view mints a License row tied to the generating Account, and
    add_machine_view now requires pasting an existing License's key rather than generating one.

    Machine.license_key remains the field that actually identifies a box (unchanged shape) --
    this table only tracks which keys have been minted, by whom, and lets add_machine_view look
    one up instead of generating fresh. A License is considered "claimed" once some Machine row
    exists with a matching license_key; there's no FK from Machine back to License, since the
    two tables are linked purely by the license_key string matching (see is_claimed below and
    ...
    def __str__(self):
        ...
        return f"{self.license_key}{claimed} — generated by {self.account.phone_number}"
